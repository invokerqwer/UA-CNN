...
parent_dir = './y_scrambling_exp'
now = datetime.now()
timestamp = now.strftime("%Y%m%d_%H%M%S")
results_dir = parent_dir + 'results/raodong/' + f'results_{csv_type}_{column_index}_{XRD_descriptor_filename}/'
...
for grid_point in tqdm(grid_points):
    ...
    for i_seed in tqdm(range(len(seeds)), position=0, leave=True): 
        ...
        y_train_scrambled = torch.from_numpy(np.random.permutation(y_train.cpu().numpy())).float().to(device)
        y_train_sub = y_train_scrambled 
...
print(f"Mean R2 Test: {mean_r2_test:.4f}, Variance R2 Test: {var_r2_test:.4f}")
print(f"Mean MAE Test: {mean_mae_test:.4f}, Variance MAE Test: {var_mae_test:.4f}")
print(f"Mean Loss Test: {mean_loss_test:.4f}, Variance Loss Test: {var_loss_test:.4f}")

results_df.loc['Mean'] = ['Mean', '', '', '', mean_r2_test, mean_mae_test, mean_loss_test]
results_df.loc['Variance'] = ['Variance', '', '', '', var_r2_test, var_mae_test, var_loss_test]

results_df.to_csv(result_path, index=False)
print(f"Updated result save to: {result_path}")

end_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
log_file.write(f"Program end time: {end_time}\n")
log_file.write(f"Mean R2 Test: {mean_r2_test:.4f}, Variance R2 Test: {var_r2_test:.4f}\n")
log_file.write(f"Mean MAE Test: {mean_mae_test:.4f}, Variance MAE Test: {var_mae_test:.4f}\n")
log_file.write(f"Mean Loss Test: {mean_loss_test:.4f}, Variance Loss Test: {var_loss_test:.4f}\n")
log_file.close()
