...
att_index = 0
results_dir = os.path.join(parent_dir, f'results/m13_att{att_index}/{timestamp}')
results_excel_file = os.path.join(results_dir, f'm13_att{att_index}_uncertainty_results.xlsx')
...
data = pd.concat([XRD_descriptor, physical_para.iloc[:,att_index]], axis=1)
...
for exp_num in range(total_experiments):
    ...
    r2_record = -math.inf
    indices = data.index
    train_indices, test_indices = train_test_split(indices, test_size=0.2, random_state=None)
    ...
    for i_seed in tqdm(range(len(selected_seeds)), position=0, leave=True): # Use seeds_len to match the initialization
        ...
        if seed not in seed_best_r2:
            seed_best_r2[seed] = float('-inf')  
        ...
        if r2_test > seed_best_r2[seed]:
            save_dir = os.path.join(results_dir, f'best_test_model_seed_{seed}')
            os.makedirs(save_dir, exist_ok=True)
            torch.save(model.state_dict(), os.path.join(save_dir, 'best_saved_model_ale_epi.pth'))
            seed_best_r2[seed] = r2_test
            with open(os.path.join(save_dir, 'best_metrics.txt'), 'w') as f:
                f.write(f"Seed: {seed}\n")
                f.write(f"Best R2: {r2_test:.6f}\n")
                f.write(f"MAE: {MAE_test:.6f}\n")
                f.write(f"MSE: {mse_loss_test:.6f}\n")
