...
class UA_CNN(nn.Module):
    def __init__(self):
        ...
        # Reduced fully connected layers to lower parameter count
        self.fc1 = nn.Linear(256, 1024)  # Reduced from 1024 to 512
        self.fc2 = nn.Linear(1024, 256)  # Reduced from 512 to 256
        self.fc3 = nn.Linear(256, 128)  # Reduced from 256 to 128
        self.fc4 = nn.Linear(128, 32)   # Reduced from 64 to 32
        self.fc5 = nn.Linear(32, 1)
        self.dropout = nn.Dropout(0.5)

        # Variance head with similar reductions
        self.fc1_var = nn.Linear(256, 1024)
        self.fc2_var = nn.Linear(1024, 256)
        self.fc3_var = nn.Linear(256, 128)
        self.fc4_var = nn.Linear(128, 32)
        self.fc5_var = nn.Linear(32, 1)
    ...
    def forward(self, x, gp):
        ...
        # For variance head
        out_var = F.relu(self.fc1_var(x))
        out_var = F.relu(self.fc2_var(out_var))
        out_var = F.relu(self.fc3_var(out_var))
        out_var = F.relu(self.fc4_var(out_var))
...
import os
import datetime
parent_dir = './parameter_var_exp/'
# Set up the new results directory
timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
results_dir = os.path.join(parent_dir, f'results_new/var5/{timestamp}')
# 设置结果Excel的文件路径
results_excel_file = os.path.join(results_dir, 'var5_uncertainty_results.xlsx')
...
for exp_num in range(total_experiments):
    ...
    y_test_ = torch.tensor(y_test.values)  # 将 Series 转换为 PyTorch Tensor
    ...
    for i_seed in tqdm(range(len(selected_seeds)), position=0, leave=True): # Use seeds_len to match the initialization
        ...
        for epoch in range(epochs):
            ...
            for minibatch in minibatches:
                ...
                batch_y_pre, _, batch_y_pre_log_var = model(batch_x,grid_point)
            ...
            model.eval()
            with torch.no_grad():
                y_test_pre, _, y_test_pre_log_var = model(x_test,grid_point)
        ...
        y_test_pre, y_test_vector, y_test_pre_log_var = model_test(x_test,grid_point)
        y_train_pre, y_train_vector, y_train_pre_log_var= model_test(x_train,grid_point)
